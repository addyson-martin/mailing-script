...
def send(fp,server,sender_mail,send_to_email,message,sent_status,otp):
    try:
        server.sendmail(sender_mail, send_to_email, message)
        fp.write(send_to_email+'\n')
        sent_status.append(send_to_email)
        response['message']="otp sent successfuly"
        response['data']=otp
        response['cond']= "True"
    except:
        response['message']="Some error occured"
        response['data']="Error check the log"
        response['cond']= "False"
sent_status=[]
...
for i in range(len_list):
    ...
    send(fp,server,sender_mail,send_to_email[i],message,sent_status,otp)
server.quit()
print(response)
fp.close()
